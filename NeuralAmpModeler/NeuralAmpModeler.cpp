...
NeuralAmpModeler::NeuralAmpModeler(const InstanceInfo& info)
    ...
    mLayoutFunc = [&](IGraphics* pGraphics) {
                    ...
                    if (fileName.GetLength()) {
                        ...
                        if (msg.size()) {
                            ...
                            if (_oNams.size() > 1) {
                                ...
                                IControl* l_oUpArrow = pGraphics->GetControlWithTag(250);  // 250 = NAM up arrow
                                l_oUpArrow->Hide(false);
                                IControl* l_oDownArrow = pGraphics->GetControlWithTag(251);  // 251 = NAM down arrow
                    ...
                    if (fileName.GetLength()) {
                        ...
                        if (retCode != dsp::wav::LoadReturnCode::SUCCESS) {
                            ...
                            if (_oIRs.size() > 1) {
                                ...
                                IControl* l_oUpArrow = pGraphics->GetControlWithTag(252);  // 252 = IR up arrow
                                l_oUpArrow->Hide(false);
                                IControl* l_oDownArrow = pGraphics->GetControlWithTag(253);  // 253 = IR down arrow
                                l_oDownArrow->Hide(false);
                            }
                            else {
        ...
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            modelArea.GetFromRight(iconWidth).GetPadded(-2.f), ClearNAM, closeButtonSVG));
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            modelArea.GetFromRightPlusOne(iconWidth).GetPadded(-2.f), namNavUp, upArrowSVG), 250);  // assign tag 250
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            modelArea.GetFromRightPlusTwo(iconWidth).GetPadded(-2.f), namNavDown, downArrowSVG), 251);  // assign tag 251
        //// testing
        //this->mNAMPath.Set("C:\\Users\\carl\\Downloads\\NeuralAmpModeler\\captures\\5152 Lead + TS9.nam");
        ...
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            irArea.GetFromRight(iconWidth).GetPadded(-2.f), ClearIR, closeButtonSVG));
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            irArea.GetFromRightPlusOne(iconWidth).GetPadded(-2.f), irNavUp, upArrowSVG), 252);  // assign tag 252
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            irArea.GetFromRightPlusTwo(iconWidth).GetPadded(-2.f), irNavDown, downArrowSVG), 253);  // assign tag 253
        //// testing
        //this->mIRPath.Set("C:\\Users\\carl\\Downloads\\NeuralAmpModeler\\captures\\Mixes\\OH 212 ZLFB H75+LYN BOLD-07.wav");
        // initially show or hide nam nav arrows
