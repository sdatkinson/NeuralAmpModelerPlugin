...
const IVColorSpec inactiveColorSpec{
    ...
    PluginColors::NAM_2.WithOpacity(0.3f), // Pressed
    PluginColors::NAM_1.WithOpacity(0.4f), // Frame
...
NeuralAmpModeler::NeuralAmpModeler(const InstanceInfo &info)
    ...
    mLayoutFunc = [&](IGraphics *pGraphics) {
        ...
        const IRECT irArea =
            content.GetFromBottom(fileHeight + 2.f).GetMidHPadded(fileWidth);
        ...
        pGraphics->AttachControl(new IBitmapControl(pGraphics->GetBounds(),
            tolexPNG, kNoParameter))->SetBlend(IBlend(EBlend::Default, 1.0));
        //  The background inside the outermost border
        pGraphics->AttachControl(new IVPanelControl(
            mainArea, "",
            style.WithDrawFrame(false).WithColor(kFG, PluginColors::NAM_1))); // .WithContrast(-0.75)
        ...
        pGraphics->AttachControl(new IVPanelControl(
            modelArea, "", style.WithDrawFrame(false).WithColor(kFG, PluginColors::NAM_1)));
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            modelArea.GetFromLeft(iconWidth).GetPadded(-5.f), loadNAM, fileSVG));
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            modelArea.GetFromRight(iconWidth).GetPadded(-8.f), ClearNAM,
            closeButtonSVG));
        pGraphics->AttachControl(
            new IVUpdateableLabelControl(
                ...
                style.WithDrawFrame(false).WithValueText(
                    style.valueText.WithSize(15.f).WithVAlign(EVAlign::Middle))),
            kCtrlTagModelName);
        // IR
        pGraphics->AttachControl(new IVPanelControl(
            irArea, "", style.WithDrawFrame(false).WithColor(kFG, PluginColors::NAM_1)));
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            irArea.GetFromLeft(iconWidth).GetPadded(-5.f), loadIR, fileSVG));
        pGraphics->AttachControl(new IRolloverSVGButtonControl(
            irArea.GetFromRight(iconWidth).GetPadded(-8.f), ClearIR,
            closeButtonSVG));
        pGraphics->AttachControl(
            new IVUpdateableLabelControl(
        ...
        IVSlideSwitchControl *noiseGateSlider =
            new IVSlideSwitchControl(ngToggleArea, kNoiseGateActive, " ", style,
                true, // valueInButton
                EDirection::Horizontal);
        pGraphics->AttachControl(noiseGateSlider);
        // Tone stack toggle
        IVSlideSwitchControl *toneStackSlider =
            new IVSlideSwitchControl(eqToggleArea, kEQActive, " ", style,
                true, // valueInButton
                EDirection::Horizontal);
        ...
        pGraphics
            ->AttachControl(
                new IVPeakAvgMeterControl(
                    inputMeterArea, "",
                    style.WithWidgetFrac(0.5).WithShowValue(false)
                    .WithDrawFrame(false).WithColor(
                        kFG, PluginColors::NAM_2.WithOpacity(0.4f)),
                ...
                new IAboutBoxControl(
                    ...
                    [](IContainerBase *pParent, const IRECT &r) {
                        ...
                        pParent->AddChildControl(new IVPanelControl(
                            IRECT(), "",
                            style
                            .WithColor(kFR, PluginColors::NAM_1.WithOpacity(0.9f))
                            .WithColor(kFG,
                                PluginColors::NAM_1.WithOpacity(0.9f))));
